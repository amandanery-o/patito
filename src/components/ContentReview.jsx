    ...
    <div className="min-h-screen bg-amber-50 pb-10">
      ...
      <main className="mx-auto max-w-2xl space-y-5 px-4 py-6">
        <section className="flex items-center gap-3 rounded-3xl bg-white p-4 shadow-sm">
          ...
          <p className="font-semibold text-gray-700">Leia com calma. Depois, tente explicar cada parte com suas próprias palavras.</p>
        </section>
        ...
        <section className="rounded-3xl bg-green-50 p-5 ring-2 ring-green-100">
          ...
          <ul className="space-y-2 text-gray-700">
            {topic.keyIdeas.map(idea => <li key={idea} className="flex gap-2"><span aria-hidden="true">✓</span><span>{idea}</span></li>)}
          </ul>
        </section>

        {hasQuestions ? (
          <button onClick={onStart} className="w-full rounded-2xl bg-blue-600 px-5 py-4 font-extrabold text-white shadow-md active:scale-95">
            Começar questões
          </button>
        ) : (
