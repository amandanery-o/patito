...
export default function HomeworkView({ items, loading, saving, error, onCreate, onUpdate, onRemove, onBack }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      /* a mensagem é exibida pelo hook */
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      <header className="bg-white shadow-sm px-4 py-3 flex items-center gap-3 sticky top-0 z-10">
        ...
        <button
          onClick={openCreate}
          className="bg-blue-500 text-white font-extrabold rounded-xl px-4 py-2 active:scale-95"
        >
          + Novo
        </button>
      </header>
      ...
      <main className="max-w-lg sm:max-w-xl mx-auto px-4 py-5 space-y-4">
        {error && (
          <p role="alert" className="bg-red-50 border border-red-200 text-red-700 rounded-2xl p-3 text-sm font-bold">
            {error}
          </p>
        )}
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="bg-white border border-blue-100 shadow-sm rounded-3xl p-5 space-y-4">
            ...
            <div>
              <label htmlFor="homework-description" className="block text-sm font-bold text-gray-600 mb-1">
                O que precisa fazer?
              </label>
              <textarea
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label htmlFor="homework-pages" className="block text-sm font-bold text-gray-600 mb-1">
                  Páginas
                </label>
                <input
                  id="homework-pages"
              ...
              <div>
                <label htmlFor="homework-due-date" className="block text-sm font-bold text-gray-600 mb-1">
                  Entregar em
                </label>
                <input
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => setShowForm(false)} className="flex-1 btn-duo-gray">
                Cancelar
              </button>
              <button disabled={saving} className="flex-1 btn-duo-green disabled:opacity-60">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {items.map((item) => (
              <article
                key={item.id}
                className={`bg-white border rounded-2xl p-4 flex gap-3 ${item.completed ? 'opacity-60 border-gray-100' : 'border-blue-100 shadow-sm'}`}
              >
                <button
                  onClick={() => onUpdate(item.id, { completed: !item.completed }).catch(() => {})}
                ...
                <div className="flex-1 min-w-0">
                  <p className={`font-bold text-gray-800 ${item.completed ? 'line-through' : ''}`}>
                    {item.description}
                  </p>
                  <p className="text-sm text-gray-500 mt-1">
                    {item.pages && `Páginas ${item.pages} · `}Entrega: {formatDate(item.due_date)}
                  </p>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => openEdit(item)} aria-label="Editar tema">
                    ✏️
                  </button>
                  <button onClick={() => setConfirmId(item.id)} aria-label="Excluir tema">
                    🗑️
                  </button>
                </div>
              </article>
