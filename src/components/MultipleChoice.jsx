...
export default function MultipleChoice({ question, onSelect }) {
  ...
  function getLabelStyle(index) {
    if (selected === null) return 'bg-gray-100 text-gray-600'
      ...
      <div className="grid grid-cols-1 gap-3">
        {question.options.map((opt, i) => (
          <button
            ...
            className={`flex items-center gap-3 w-full text-left p-4 rounded-xl border-2 font-semibold transition-all duration-200 ${getStyle(i)}`}
          >
