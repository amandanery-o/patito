  ...
  return (
    <div className="flex flex-col items-center gap-0 pb-8">
      {topics.map((topic, index) => {
        ...
        return (
          <div key={topic.id} className="flex flex-col items-center w-full max-w-xs">
            ...
            <div className="flex items-center gap-4 w-full px-4">
              ...
              >
                {completed
                  ? (stars === 3 ? '⭐' : stars === 2 ? '🌟' : '✅')
                  : <span className="font-extrabold text-base">{index + 1}</span>
                }
              </div>
              ...
              <button
                ...
                className={`flex-1 rounded-2xl p-4 text-left transition-all active:scale-95
                  ${completed
                    ...
                    : 'bg-white border-2 border-gray-200 hover:border-blue-300 hover:shadow-md shadow-sm'
                  }`}
              >
                <p className={`font-extrabold text-base ${completed ? 'text-green-700' : 'text-gray-800'}`}>
                  {topic.title}
                </p>
