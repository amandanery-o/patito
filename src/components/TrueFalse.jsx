    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <button
          ...
          className={`flex flex-col items-center justify-center gap-2 p-6 rounded-2xl border-2 transition-all duration-200 ${getStyle(true)}`}
        >
          <span className="text-4xl">✅</span>
          <span className="text-base font-bold text-gray-700">Verdadeiro</span>
        </button>
        <button
          onClick={() => handleSelect(false)}
          className={`flex flex-col items-center justify-center gap-2 p-6 rounded-2xl border-2 transition-all duration-200 ${getStyle(false)}`}
        >
          <span className="text-4xl">❌</span>
          <span className="text-base font-bold text-gray-700">Falso</span>
        </button>
      </div>
      {selected !== null && (
        <div className={`mt-3 p-3 rounded-xl text-sm font-medium ${selected === question.correct ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
