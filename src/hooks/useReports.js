...
export function useReports(userId = null, repository = createProblemReportRepository()) {
  ...
  async function addReport(input) {
    ...
    setReports(previous => { const next = [pending, ...previous]; save(storageKey, next); return next })
    try {
      await repository.submit(payload)
      setReports(previous => {
        const next = previous.map(report => report.correlationId === payload.correlationId ? { ...report, status: 'sent' } : report)
        save(storageKey, next)
        return next
      })
    } catch (error) {
      setReports(previous => {
        const next = previous.map(report => report.correlationId === payload.correlationId ? { ...report, status: 'error' } : report)
        save(storageKey, next)
        return next
      })
      throw error
